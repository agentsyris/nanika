...
class CalmOpsSearchService:
    def __init__(self):
        self.api_key = os.getenv('8c05dfce5bdd4f7a982c7c4604208a44e2502179')
        self.base_url = "https://google.serper.dev/search"
        
        # Target markets - avoiding SF as you mentioned
        self.target_markets = {
            'new_york': {
    ...
    async def validate_problem_search(self, market: str = 'all', week_number: int = 1) -> Dict:
        """
        ...
        # Search queries specifically for finding companies with workflow pain
        validation_queries = [
            f"creative team growing pains {market} 50-200 employees",
            f"marketing operations challenges {market}",
            f"Series A Series B companies {market} hiring creative",
            f"fast growing startups {market} marketing team",
            f"companies hiring creative directors {market}",
            f"marketing workflow automation {market}",
            f"project management creative teams {market}"
        ]
        
        for query in validation_queries:
            try:
                search_results = await self._search(query)
                prospects = self._extract_validation_prospects(search_results, market)
        ...
        for prospect in results['prospects'][:10]:
            decision_makers = await self._find_decision_makers(prospect)
            results['decision_makers'].extend(decision_makers)
            
        return results
    
    async def find_pilot_candidates(self, validated_prospects: List[Dict]) -> Dict:
        """
        ...
        for prospect in validated_prospects[:5]:  # Top 5 validated prospects
            ...
            queries = [
                ...
                f"{company_name} project delays issues",
                f"site:linkedin.com/company {company_name}",
                f"{company_name} glassdoor reviews workflow"
            ]
    ...
    async def content_marketing_research(self, case_study_client: str) -> Dict:
        """
        Week 4-5 Priority: Research for content marketing launch
        """
        content_ideas = []
        
        # Research trending topics in workflow optimization
        trend_queries = [
            "creative team workflow optimization 2024 2025",
            "marketing operations best practices",
            "project management creative agencies",
            "remote creative team collaboration",
            f"{case_study_client} innovation technology"
        ]
        
        for query in trend_queries:
            try:
                results = await self._search(query)
                topics = self._extract_content_topics(results)
                content_ideas.extend(topics)
            except Exception as e:
                print(f"Content research error: {e}")
                
        return {
            'content_ideas': content_ideas,
            'trending_topics': self._identify_trends(content_ideas),
            'competitor_content': await self._research_competitors()
        }
    
    async def scale_outreach_targets(self, 
                                    market: str,
        ...
        # Industry-specific searches for each market
        ...
        for sector in market_config['sectors']:
            ...
            try:
                results = await self._search(query, num_results=20)
                companies = self._extract_outreach_targets(results, market, sector)
                targets.extend(companies)
            except Exception as e:
                print(f"Outreach search error: {e}")
                
        # LinkedIn specific searches for decision makers
        for target in targets[:target_count]:
            linkedin_query = f"site:linkedin.com/in creative director {target['company_name']} OR CMO OR marketing operations"
            try:
                results = await self._search(linkedin_query)
    ...
    async def _search(self, query: str, num_results: int = 10) -> Dict:
    ...
    def _extract_company_name(self, result: Dict) -> str:
        ...
        # Simple extraction - you'd want to make this more sophisticated
        title = result.get('title', '')
    ...
    async def _find_decision_makers(self, prospect: Dict) -> List[Dict]:
